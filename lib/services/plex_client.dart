...
class PlexClient {
  ...
  Future<({String url, String sessionId})> getTranscodeDownloadUrl(
    ...
    // Validate the decision response — the server may reject the transcode request
    if (decisionResponse.statusCode != null && decisionResponse.statusCode! >= 400) {
      throw Exception('Transcode decision failed with status ${decisionResponse.statusCode} for $ratingKey ($quality)');
    }
    if (decisionResponse.data is Map<String, dynamic>) {
      final container = (decisionResponse.data as Map<String, dynamic>)['MediaContainer'] as Map<String, dynamic>?;
      final generalDecision = container?['generalDecisionCode'] as int?;
      final generalDecisionText = container?['generalDecisionText'] as String?;
      if (generalDecision != null && generalDecision >= 2000) {
        throw Exception('Transcode rejected: ${generalDecisionText ?? 'code $generalDecision'} for $ratingKey ($quality)');
      }
    }
  ...
  Future<({double progress, bool complete})?> getTranscodeSessionProgress(String sessionId) async {
    try {
      ...
      // Match by session key containing our sessionId
      final matched = sessions.where((s) {
        final key = s['key'] as String?;
        return key != null && key.contains(sessionId);
      }).toList();
      if (matched.isNotEmpty) {
        return _extractTranscodeProgress(matched.first);
      }

      // Fallback: prefer HTTP-protocol sessions (download transcodes use protocol=http)
      ...
      // Last resort: use the only session if there's just one
